...
public class EnemyGrid : MonoBehaviour
{
    ...
    private Dictionary<Enemy, Tuple<int, int>> lastPositions;
    //los "contenedores"
    private HashSet<Enemy>[,] buckets;
    ...
    readonly public Enemy[] Empty = new Enemy[0];
    #endregion

    static EnemyGrid _instance;
    public static EnemyGrid Instance { get { return _instance; } private set { } }
    ...
    void Refresh()
    {
        lastPositions = new Dictionary<Enemy, Tuple<int, int>>();
        buckets = new HashSet<Enemy>[width, height];

        //creamos todos los hashsets
        for (int i = 0; i < width; i++)
            for (int j = 0; j < height; j++)
                buckets[i, j] = new HashSet<Enemy>();
        ...
        {
            e.UpdateDelegate += UpdateEntity;
            //e.OnMove += UpdateEntity;
            UpdateEntity(e);
        }
    }

    public void UpdateEntity(Enemy entity)
    {
    ...
    public IEnumerable<Enemy> Query(Vector3 aabbFrom, Vector3 aabbTo, Func<Vector3, bool> filterByPosition)
    {
    ...
    void OnDestroy()
    {
        var ents = RecursiveWalker(transform).Select(x => x.GetComponent<Enemy>()).Where(x => x != null);
        foreach (var e in ents) e.UpdateDelegate -= UpdateEntity;
    }
    ...
    private void OnDrawGizmos()
    {
        ...
        if (!activatedGrid)
        {
            IEnumerable<Enemy> allElems = Enumerable.Empty<Enemy>();
